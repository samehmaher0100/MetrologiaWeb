...
Public Class pgHcal_Cam
    ...
    Public Function Grados_de_Libertad(uer As String, Incertidumbre As String, ures As String, urescero As String, UEXC As String, UHist As String, Upat As String, UmB As String, udmp As String, udmcom As String) As String
        ...
        respuesta = uer ^ 4 / (Incertidumbre ^ 4 / (3 - 1) + ures ^ 4 / 100 + urescero ^ 4 / 100 + UEXC ^ 4 / 100 + Upat ^ 4 / 100 + udmp ^ 4 / 100 + UmB ^ 4 / 100 + udmcom ^ 4 / 100)
        Return respuesta
    End Function
    ...
    Public Sub Histersis(Proyecto As String, whis As String, Hist As String)
        Dim datos As New DataSet
        datos = Formula.Histersis_Camionera(Proyecto, whis, Hist) '  .Clientes_Registrados("Codigo", Request.QueryString.Get("Codigo"))
        Dim i As Integer = 0
        ReDim v_Histe(100)
        For Each row As DataRow In datos.Tables(0).Rows
    ...
    Private Sub Imprimir()
        ...
        Dim host As String = Request.Url.ToString().Replace("pgHcal_Cam.aspx", "Reportes/Frm_ReportesClaseCamioenra.aspx")
    ...
    Private Sub Cargar_Informacion(Codigo_Proyecto As String, Item As String)
        ...
        Dim v_Ures As Double()
        Dim v_Uexc As Double()
        'Dim v_Histe As String()
        Dim v_Res0 As Double()
        Dim v_Vpat As Double()
        Dim v_mb As Double()
        Dim v_Smp As Double()
        Dim v_Smcom As Double()
        '******************************************************
        ...
        Str_a = "select estbpr from balxpro where IdeComBpr='" & Codigo_Proyecto & Item & "'"
        ...
        '-----------------llenar la capacidad maxima calibrada-----------------

        Dim str_lugcal1 As String = "select MAX(CARpCA) from PCarga_Cab where idecombpr ='" & IdeComBpr_G & "'
"
        Dim ObjCmd_lugcal1 As SqlCommand = New SqlCommand(str_lugcal1, ccn)
        Dim ObjReader_lugcal1 = ObjCmd_lugcal1.ExecuteReader
        While (ObjReader_lugcal1.Read())
            Lbl_CALIBRADA.Text = ObjReader_lugcal1(0).ToString()
        End While
        ObjReader_lugcal1.Close()
        ...
        While (ObjReader.Read())
            ...
            Dim contador_prueba As Integer
            ...
            Dim StrSql As String = "SELECT PCarga_Cab.IdeComBpr,PCarga_Cab.NumPca,PCarga_Cab.CarPca," &
                                 ...
                                 "SUBSTRING(PCarga_Det.codpca_c,8,len(PCarga_Det.codpca_c))=PCarga_Cab.NumPca  order by NumPca asc"  '"'"
...
aqui:
            ...
            For i = 0 To dimension - 1
                ...
                tCell.InnerText = coma(Convert.ToDouble(v_Histe(i)).ToString("0.000000000")) 'coma(Convert.ToString(v_Histe(i)))
                tRow.Cells.Add(tCell)
            ...
            For i = 0 To dimension - 1
                ...
                'µ(I)
                tCell = New HtmlTableCell
                Dim resss = coma((Math.Sqrt(Val(cuadrado_indicacion(i)))).ToString("0.000000000"))
                tCell.InnerText = coma((Math.Sqrt(Val(cuadrado_indicacion(i)))).ToString("E5")) 'coma(ccv_sal.ToString("E5"))
                tRow.Cells.Add(tCell)
                ''µ(mref)
                'tCell = New HtmlTableCell
                ...
                End If
                vector_uref(i) = umref
                tCell.InnerText = coma(Convert.ToSingle(umref).ToString("E5"))  'umref
                tRow.Cells.Add(tCell)
                'µ(Er)
                tCell = New HtmlTableCell
                ...
                tCell.InnerText = uer(i) 'coma(Convert.ToSingle(uer(i)).ToString("E5")) 'formateo(uer(i), 4)
                tRow.Cells.Add(tCell)
                'Oeff
                tCell = New HtmlTableCell
                tCell.InnerText = Convert.ToDouble(coma(Grados_de_Libertad(uer(i), desviacion, v_Ures(i), v_Res0(i), v_Uexc(i), v_Histe(i), v_Vpat(i), v_mb(i), v_Smp(i), v_Smcom(i)))).ToString("0.0000000")
                tRow.Cells.Add(tCell)
                'k
                Dim Grados_Libertad As Double = Convert.ToDouble(coma(Grados_de_Libertad(uer(i), desviacion, v_Ures(i), v_Res0(i), v_Uexc(i), v_Histe(i), v_Vpat(i), v_mb(i), v_Smp(i), v_Smcom(i)))).ToString("0.0000")
                Dim valk As Double = Valor_K(Grados_Libertad)   'valk
                tCell = New HtmlTableCell
                tCell.InnerText = valk.ToString("0.00") 'coma(valk.ToString("E1"))    'valk
                tRow.Cells.Add(tCell)
                k(i) = valk '.ToString("0.00000")  'Format(Convert.ToDouble(valk), "#.##0")
                'U exp
                tCell = New HtmlTableCell
                Dim uexp As Double = uer(i) * k(i)
                tCell.InnerText = coma(uexp.ToString("E1"))  'coma((Val(uer(i)) * Val(k(i))).ToString("0.000000")) 'coma((uer(i) * k(i)).ToString("E1"))
                tRow.Cells.Add(tCell)
                U_reporte(i) = uer(i) * (Format(Convert.ToDouble(k(i)), "#.###0")) 'uexp 'coma(uexp.ToString("0.000000")) 'coma((uer(i) * k(i)).ToString("0.000000")) 'coma((uer(i) * k(i)).ToString("E1"))
                'creo Row
            ...
            For i = 0 To dimension - 1
                ...
                'U exp
                tCell = New HtmlTableCell
                tCell.InnerText = Convert.ToDouble(U_reporte(i)).ToString("0.000") 'Format(Convert.ToDouble(U_reporte(i)), "#.##0") 'Convert.ToDouble(U_reporte(i)).ToString("0.000")
                tRow.Cells.Add(tCell)
                'creo Row
            ...
            For i = 0 To dimension - 1
                ...
                "" & Replace(vector_lecdsc(i), ",", ".") & "," & Replace(vector_errdsc(i), ",", ".") & "," & Replace((Format(Convert.ToDouble(k(i)), "#.#0")), ",", ".") & "," & Replace(Convert.ToDouble(U_reporte(i)).ToString("0.000"), ",", ".") & ")"
