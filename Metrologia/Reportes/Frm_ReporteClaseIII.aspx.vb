...
Public Class Frm_ReporteClaseIII
    ...
    '***Grados de libertad*****
    Public Function Grados_de_Libertad(uer As String, Incertidumbre As String, ures As String, urescero As String, UEXC As String, UHist As String, Upat As String, UmB As String, udmp As String, udmcom As String) As String
        Dim respuesta As Double
        '=         E111 ^ 4 / (        $E$43 ^ 4 /($D$38-1)+ C83  ^ 4  /100 + G83      ^4   /100 '+E83  ^ 4  /100 + 'F83  ^ 4  /100 + I83  ^ 4  /100 + K83 ^ 4  /100 + L83  ^ 4  /100 + N83    ^ 4  /100)
        respuesta = uer ^ 4 / (Incertidumbre ^ 4 / (3 - 1) + ures ^ 4 / 100 + urescero ^ 4 / 100 + UEXC ^ 4 / 100 + UHist ^ 4 / 100 + Upat ^ 4 / 100 + UmB ^ 4 / 100 + udmp ^ 4 / 100 + udmcom ^ 4 / 100)
    ...
    Private Sub Imprimir()
        ...
        Dim host As String = Request.Url.ToString()

        ' create a new pdf document converting an url
        Dim doc As PdfDocument = converter.ConvertUrl(host)

        ' save pdf document
        doc.Save(Response, False, Lbl_NCertificado.Text & ".pdf")
    ...
    Private Sub Cargar_Informacion(Codigo_Proyecto As String, Item As String)
        ...
        'Dim respu As String = excel.Datosexcel(Codigo_Proyecto & Item)
        ...
        While (ObjReader.Read())
            ...
            ' Dim nu_desv As Double = desviacion / Math.Sqrt(3)'revision numero 10
            'revision numero 11
            ...
            While (ObjReader_a.Read())
                'Errores 
                Dim error_a As String = ObjReader_a(5)
                Dim error_d As String = ObjReader_a(6)
                '   Label5.Text = error_a & " -- " & error_d
                'masa convencional
...
aqui:
                ...
                If unidad = "[ g ]" Then
                    vector_emp_patron(pos_vector) = coma(emp_pat)
                    'Revision N11 mdoficado por angel
                    vector_u_std_patron(pos_vector) = coma((inc_std_pat))
                    'Revision N11 mdoficado por angel
                    'vector_u_deriva_patron(pos_vector) = coma(Math.Sqrt(inc_der_pat)) ' esto ya no va en la revision 11
                ...
                Else
                    ...
                    'Revision N11 mdoficado por angel
                    'vector_u_deriva_patron(pos_vector) = Val(coma(Math.Sqrt(inc_der_pat))) / 1000' esto ya no va en la revision 11
                ...
                tRow.Cells.Add(tCell)
                '************************************************Cambio para la condicion de la histersis hmax  y carga de h max************************************

                'Hmax
                tCell = New HtmlTableCell()
                Dim pruebas As Double
                Dim maxhisteresis As Double ' = ""
                ...
                While (ObjReader_d.Read())
                    ...
                    '  Hicertidumbre_Maxima = coma(formateo(ObjReader_d(0).ToString(), 1)) 'formateo(coma((ObjReader_d(0).ToString())), 1)
                    pruebas = ObjReader_d(0)
                    ' hmax = ObjReader_d(1)
                End While
                ObjReader_d.Close()
                If coma(campo_va) = 200 Then
                    Dim sd = ObjReader_a(4).ToString()
                    Dim sd1 = ObjReader_a(3).ToString()

                End If

                Dim histeresis As Double = Math.Abs(Val(coma(formateo((Val(coma((ObjReader_a(4).ToString()))) - Val(coma((ObjReader_a(3).ToString())))), 1))))
                '                Dim histeresis As String = Math.Abs(Val(coma(formateo(Math.Abs(Val(coma((ObjReader_a(4).ToString()))) - Val(coma((ObjReader_a(3).ToString())))), 1))))




                hmax = v_Hmax(pos_vector)
                tCell.InnerText = formateo(histeresis, 1)


                'If histeresis = maxhisteresis Then
                '    hmax = histeresis
                '   tCell.InnerText = formateo(histeresis, 1)

                '    'hmax = v_CHmax(i)
                '    'tCell.InnerText = formateo(hmax, 1)

                'Else
                '    Dim cero As String = "0"
                '    tCell.InnerText = formateo(cero, 1) 'coma(Val(histeresis)) & "=" & coma(Val(maxhisteresis)) '
                '    hmax = 0
                'End If
                tRow.Cells.Add(tCell)
                'carga de HMax
                ...
                carga_hmax = v_CHmax(pos_vector)
                tCell.InnerText = carga_hmax  'carga_hmax.ToString
                tRow.Cells.Add(tCell)


                '************************************************fin del  Cambio para la condicion de la histersis hmax  y carga de h max************************************
                'evaluación de emp
                tCell = New HtmlTableCell()
                ...
                'Label5.Text = Label5.Text & error_a & " <= " & emp_recal & ") And (" & error_d & " <= " & emp_recal & "||||=>next: "
                'If (((Math.Abs(Val((coma(ObjReader_a(5).ToString()))))) <= emp_recal) And ((Math.Abs(Val((coma(ObjReader_a(6).ToString()))))) <= emp_recal)) Then
                'If ((Math.Abs(ObjReader_a(5)) <= emp_recal) And ((Math.Abs(ObjReader_a(6)) <= emp_recal)) Then
            ...
            lblIncertidumbreHist.Text = max_inc_hist 'coma(hist_tot)
            lblIncertidumbreHist.Text = lblIncertidumbreHist.Text.Replace(",", ".")
            ...
            t1.InnerHtml = "<img src='/images/inc_conveccion.png'/ style='max-width: 300px; max-height: 300px'>"
            tRowTitle.Cells.Add(t1)
            nutabl2.Rows.Add(tRowTitle)
            ...
            For i = 0 To dimension - 1
                ...
                'CAMBIO REALIZADO X ANGEL 2019-08-21 'AA
                ...
                'tRow.Cells.Add(tCell)
                'Dim histe As Double
                'If (Val(coma(vector_convencional(i))) * hist_tot) > u_res_d Then

                '    histe = u_res_d 'Val(coma(lblIncertidumbreHist.Text)) * Val(coma(vector_convencional(i)))
                tCell.InnerText = coma(Convert.ToString(v_Histe(i)))
                tRow.Cells.Add(tCell)
                'fin CAMBIO REALIZADO X ANGEL 2019-08-21
                cuadrado_indicacion(i) = cuadrado_indicacion(i) + Val(coma(v_Histe(i))) ^ 2
                ...
                Dim pruebas2 As Double = (Math.Sqrt(3)).ToString("E5")
            ...
            For i = 0 To dimension - 1
                ...
                'Label6.Text = cuadrado_indicacion(i)
                tRow.Cells.Add(tCell)
                'µ(mref)
                tCell = New HtmlTableCell
                Dim umref As String = ""
                ...
                Else
                    ...
                    'umref = coma(Math.Sqrt(cuadrado_patron(i)).ToString("0.000000000"))
                    Dim umref_d As Double = Math.Sqrt(cuadrado_patron(i))
                    'Label6.Text = Label6.Text & "otra" & i & " :" & cuadrado_patron(i) & "|| "
                    umref = coma(umref_d.ToString("0.000000000"))
                End If
                vector_uref(i) = umref
                ...
                'If Val(lblIncertidumbreRep.Text) > 0 Then
                ...
                tCell = New HtmlTableCell
                'tCell.InnerText =' "hola" 'coma(Oeff.ToString("E3"))
                'REVISION N11
                tCell.InnerText = Grados_de_Libertad(uer(i), desviacion, v_Ures(i), v_Res0(i), v_Uexc(i), v_Histe(i), v_Vpat(i), v_mb(i), v_Smp(i), v_Smcom(i))
                tRow.Cells.Add(tCell)
                'k
                ...
                Dim Grados_Libertad As String = Grados_de_Libertad(uer(i), desviacion, v_Ures(i), v_Res0(i), v_Uexc(i), v_Histe(i), v_Vpat(i), v_mb(i), v_Smp(i), v_Smcom(i))

                valk = Valor_K(Grados_Libertad)   'valk
                tCell = New HtmlTableCell
                tCell.InnerText = Format(Convert.ToDouble(valk), "#.##0") 'Math.Ceiling((Convert.ToDouble(valk)) * 1000D) / 1000D
                tRow.Cells.Add(tCell)
                k(i) = Format(Convert.ToDouble(valk), "#.##0")
                'U exp
                tCell = New HtmlTableCell
                Dim uexp As Double = uer(i) * Val(k(i))
                tCell.InnerText = coma(uexp.ToString("E1"))
                tRow.Cells.Add(tCell)
            ...
            For i = 0 To dimension - 1
                ...
                'k
                tCell = New HtmlTableCell
                tCell.InnerText = Format(Convert.ToDouble(k(i)), "#.#0") '(Math.Ceiling((Convert.ToDouble(k(i))) * 100D) / 100D) '  k(i)
                'Return Math.Ceiling(MathNet.Numerics.ExcelFunctions.TInv(1 - 0.95, var_calculo) * 100D) / 100D
            ...
            For i = 0 To dimension - 1
                ...
                "" & Replace(vector_lecdsc(i), ",", ".") & "," & Replace(vector_errdsc(i), ",", ".") & "," & Replace((Format(Convert.ToDouble(k(i)), "#.#0")), ",", ".") & "," & Replace(U_reporte(i), ",", ".") & ")"
            ...
            While (ObjReader1.Read())
                ...
                While (ObjReader2.Read())
                    ...
                    lblDifPos1_2.Text = formateo(Math.Abs(Val(lblValPos1.Text) - Val(lblValPos1.Text)), 1)
                    vector_exct(0) = Val(lblDifPos1_2.Text)
                    lblDifPos2_2.Text = formateo(Math.Abs(Val(lblValPos1.Text) - Val(lblValPos2.Text)), 1)
                    vector_exct(1) = Val(lblDifPos2_2.Text)
                    lblDifPos3_2.Text = formateo(Math.Abs(Val(lblValPos1.Text) - Val(lblValPos3.Text)), 1)
                    vector_exct(2) = Val(lblDifPos3_2.Text)
                    lblDifPos4_2.Text = formateo(Math.Abs(Val(lblValPos1.Text) - Val(lblValPos4.Text)), 1)
                    vector_exct(3) = Val(lblDifPos4_2.Text)
                    lblDifPos5_2.Text = formateo(Math.Abs(Val(lblValPos1.Text) - Val(lblValPos5.Text)), 1)
            ...
            If consust.Equals("n") Or consust.Equals("s") Then
                ...
                lblval_upat_eii.Text = coma(Val(coma(inc_patron_eii)).ToString("E5"))
                lblval_emppat_eii.Text = coma(Val(coma(emp_patron_eii)).ToString("E5"))
                ...
                lblval_udmp_eii.Text = coma(Val(coma(inc_deriva_eii)).ToString("E5"))
                ...
                If unidad = "[ g ]" Then
                    ...
                    engreii = Val(lblval_crgpat_eii.Text)
                Else
                    'CAMBIO AA

                    'engreii = Val(crg_conv_eii) * 1000
                    engreii = Val(lblval_crgpat_eii.Text) * 1000
            ...
            'CALCULO DEL ERROR NORMALIZADO  angel Aucancela 



            'Fin del CALCULO DEL ERROR NORMALIZADO
    ...
    '    'If ddlMax_i.SelectedIndex > 0 Then
